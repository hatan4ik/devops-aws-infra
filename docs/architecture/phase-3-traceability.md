...
| 3.2: Organizations accounts, OUs, Control Tower/custom decision, SCPs, vending | [Account/OU diagram](README.md); [ADR 0019](../adr/0019-direct-organizations-account-vending.md); [security controls](network-security.md). | Direct Organizations control plane, OUs, and baseline SCPs are delivered; Control Tower/AFT is deferred and account vending remains empty by design. |
...
Phase 4–7 target source and traceability exist in their dedicated matrices. The
single-account sandbox delivery identities, backends, roots, and GitHub plans/
applies are live; account vending and the multi-account target remain
separately gated.
