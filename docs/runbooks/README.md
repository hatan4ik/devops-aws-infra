...
| [GitHub Actions Terraform delivery](github-actions-delivery.md) | Plan, apply, and detect drift through the active root-specific OIDC workflows. | Reviewed plan, protected `main`, explicit apply confirmation, protected environment, and no-change reconciliation plan. |
...
| [Sandbox delivery IAM adoption](sandbox-delivery-iam-adoption.md) | Adopt the sandbox OIDC provider, roles, and delivery policies into Terraform; retire historical CloudFormation. | Reviewed import plan, protected OIDC plan, retained-resource stack deletion, and no-change drift proof. |
| [Sandbox workload GitOps delivery](sandbox-workload-delivery.md) | Deliver private Fargate services from immutable images through the dedicated workload root. | Reviewed service plan, protected apply, private task evidence, image-digest evidence, and no-change reconciliation plan. |
