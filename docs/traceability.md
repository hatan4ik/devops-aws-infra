...
| Security and isolated state | [ADR 0008](adr/0008-security-and-state.md), [state backend module](../terraform/modules/internal/state-backend/), [legacy bootstrap inventory](architecture/legacy-state-bootstrap.md) | No committed backend configuration, static key, or state. A separate legacy bootstrap exists outside the canonical delivery tree. | Canonical implementation is candidate-only; legacy-resource disposition is pending an owner decision. |
