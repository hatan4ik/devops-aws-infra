...
2. **Initialized** the Terraform provider in `roots/shared-services/us-east-1/prod`.
...
* **S3 Bucket**: `aws_s3_bucket.state` (Dynamically named `ses-tf-state-shared-<random-hex>`).
* **S3 Protections**: 
...
* **DynamoDB Table**: `aws_dynamodb_table.lock` (For state locking, named `ses-tf-lock-table`).
