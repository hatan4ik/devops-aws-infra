#!/usr/bin/env bash
# Configures non-secret role-ARN variables only after the reviewed IAM policy
# stack is active. AWS credentials are never written to GitHub.
set -euo pipefail
...
usage() {
  cat <<'USAGE'
Usage: scripts/configure-sandbox-network-github.sh [options]

Stores only role ARNs as GitHub variables:
  repository variable AWS_SANDBOX_NETWORK_PLAN_ROLE_ARN
  dev environment variables AWS_SANDBOX_NETWORK_APPLY_ROLE_ARN and
  AWS_SANDBOX_NETWORK_DRIFT_ROLE_ARN
...
gh variable set AWS_SANDBOX_NETWORK_PLAN_ROLE_ARN --repo "$repository" --body "$plan_role_arn"
gh variable set AWS_SANDBOX_NETWORK_APPLY_ROLE_ARN --repo "$repository" --env dev --body "$apply_role_arn"
gh variable set AWS_SANDBOX_NETWORK_DRIFT_ROLE_ARN --repo "$repository" --env dev --body "$drift_role_arn"

printf 'Configured non-secret sandbox-network role ARN variables for %s.\n' "$repository"
